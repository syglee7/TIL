...
function pay(pay_url) {
    document.write("<body><div class='pay_div'>" +
        "<h1>Quick Checkout</h1>" +
        "<div class='result' id='result' name='result'> " +
        "<div class='overview'> Payment URL : <a href=" +pay_url+ ">"+ pay_url +"</a> </div><br> " +
        "<div id='qrcode'></div>  <input type='hidden' id='amount' value='888888'> <br> " +
        "<input type='button' value='Transaction Details' onclick='check_details()'  id='check' class='check'><div class='details'></div></div>" +
        "<script src='./main.js'></script> <link rel='stylesheet' type='text/css' href='style.css' /></body>");
...
function get_payment_detail() {
    try {
        ...
        $('.details').append('<h2>Invoice :</h2><ul><li> Network : '+network+'</li>' +
            '<li>Transaction Timestamp : '+time+'</li><li>Expiration : '+expires+'</li>' +
            '<li>Merchant data : '+merchant_data+'</li><li> Merchant Signature verification: '+verified+'</li>' +
            '<li>Memo: '+memo+'</li><li> Total : 0.0088888</li>');
